...
Success Criterion~5 holds. Across all four benchmark modes of
Section~12 and both draws, every generated problem's objective agrees
with the reference solver to within $10^{-6}$ relative --- 18 of 18
comparisons, with observed gaps between $10^{-11}$ and $10^{-9}$. The
five conic KKT residuals of Section~6 at termination lie between
$10^{-10}$ and $10^{-9}$, an order of magnitude tighter than the
stopping tolerance.

\subsection{Robustness}

Thirteen instance families, three seeds each, twenty assets: all
thirty-nine solve, with residuals certified by Section~6's criterion.
Outcomes are classified into four rather than reported as a status
string, because a stop that names what happened and one that does not
...
verdict & count\\
\hline
solved (optimal, and the residuals certify it) & 39\\
...
That last result reversed an earlier one, and how it reversed is
worth recording. One family --- a deliberately badly scaled instance
--- did stall, and equilibration did convert the stall into an
optimum. The diagnosis was wrong. The routine that repairs conic
feasibility by setting $t=\|Lx\|_2$ required the cone's head row to
select its variable with a coefficient of exactly one; no rescaled
instance satisfies that, so the retraction was silently unavailable
and an iterate on the cone's boundary could not move. Equilibration
was perturbing the head row's coefficient, not the conditioning. With
the restriction removed the family solves to $10^{-11}$ unaided, and
equilibrating it makes the residual five orders of magnitude worse.
